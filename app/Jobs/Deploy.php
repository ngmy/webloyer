...
class Deploy extends Job implements ShouldQueue
{
    ...
    const RUNNING_DEPLOYMENT_STATUS = 3;
    ...
    )
    {
        ...
        if ($deployment->status === null && (!$project->lastProjectDeployment()->number || $deployment->number === $project->lastProjectDeployment()->number)) {
            ...
            $tmp['status'] = self::RUNNING_DEPLOYMENT_STATUS;
            $project->updateDeployment($tmp);

            $process->run(function ($type, $buffer) use (&$tmp, $project, $deployment, $process) {
                $tmp['message'] .= $buffer;
                $tmp['process_id'] = $process->getPid();
                $project->updateDeployment($tmp);
            });
