...
namespace Webloyer\Infra\Db\Eloquents\Project;
...
class Project extends Model implements ProjectDomainModel\ProjectInterest
{
    ...
    {
        return $this->belongsTo(Eloquents\Server\Server::class);
    }
    ...
    {
        $serverOrm = Eloquents\Server\Server::ofId($serverId)->first();
    ...
    public function toEntity(): ProjectDomainModel\Project
    {
        ...
        return ProjectDomainModel\Project::of(
            $this->uuid,
            $this->name,
            $this->recipes->map(function (Eloquents\Recipe\Recipe $recipe) {
                return $recipe->uuid;
            })->toArray(),
            $this->server->uuid,
            $this->repository,
